...
function createEmail(body){
    ...
    const emailActivity = `<b>Associated Deviation - </b><em>${body.SourceId}</em> </br>
...
function getDeviationList(filepath, regExSearch) {
    const status = 4;
    const file = config.uploaded + filepath;
    const fields = ['SourceId', '_name', 'TKName', 'TKTarg', 'TKChamp', 'TKStat'];
    ...
    Deviation.find({})
        ...
        .exec(function(err, collection) {
            ...
            Task
                .where('TKStat').lte(4)
                .where({$or: [{TKChamp : regExSearch }, {TKName: regExSearch}, {SourceId: regExSearch}]})
                .select({SourceId:1, TKName:1, TKTarg:1, TKChamp:1, TKStat:1})
                .exec(function(err, coll) {
                    ...
                    const reformattedArray = coll.map(function(obj){
                        ...
                        const _tasks = collection.find(deviation => deviation.dvNo === obj.SourceId);

                        if (typeof _tasks === 'object') {
                            const _name = _tasks.dvMatName;
                            return {TKName, _name, TKTarg, TKChamp, TKStat, SourceId};
                        }
